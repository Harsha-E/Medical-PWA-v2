...
export default class RegisterView {
  async render() {
    ...
    this.container.innerHTML = `
      <div class="w-full max-w-md p-8 rounded-3xl bg-[#0a0407]/40 backdrop-blur-3xl border border-[#7f2f5d]/30 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-fade-in-up">
        <div class="text-center mb-8">
          <h2 class="text-3xl font-display font-semibold text-white tracking-tight">Initialize</h2>
          <p class="text-[#ffb88c] text-sm mt-2 font-mono uppercase tracking-widest">Create secure ledger</p>
        </div>
        ...
        <form id="register-form" class="space-y-4">
          ...
          <div>
            ...
            <input type="password" id="password" autocomplete="new-password" placeholder="Create Password" required class="w-full px-5 py-4 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:outline-none focus:border-[#ffb88c]/50 focus:bg-white/10 transition-all font-sans">
          </div>
          
          <button type="submit" id="email-btn" class="w-full py-4 rounded-xl bg-linear-to-r from-[#ffd9b5] to-[#ffb88c] text-[#4a1532] font-mono text-xs font-bold uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all shadow-[0_0_20px_rgba(255,217,181,0.3)] mt-2">
            Deploy Protocol
          </button>
        </form>
        ...
        <button id="google-auth" type="button" class="w-full flex items-center justify-center gap-3 py-4 rounded-xl bg-white/5 border border-white/10 text-white font-mono text-xs uppercase tracking-widest hover:bg-white/10 active:scale-95 transition-all backdrop-blur-md">
        ...
        <p class="mt-8 text-center text-sm text-gray-500">
          Already secured? <a href="#/login" class="text-[#ffb88c] hover:text-[#ffd9b5] transition-colors font-medium">Access Portal</a>
        </p>
      </div>
    `;
