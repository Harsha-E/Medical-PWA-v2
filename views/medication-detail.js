    ...
    this.container.innerHTML = `
      ...
      <main class="scroll-area px-6 pb-24">
        ...
        <section class="mb-8 flex gap-4">
          <button onclick="window.location.hash='#/interactions?add=${id}'" class="flex-1 bg-[#1a0a12] border border-[#ffb88c]/50 text-[#ffb88c] rounded-2xl py-4 px-2 text-xs font-bold uppercase tracking-widest hover:bg-[#ffb88c]/10 active:scale-95 transition-all text-center flex flex-col items-center justify-center gap-2">
  ...
  renderError(msg) {
    return `
      <div class="flex flex-col items-center justify-center min-h-[80vh] px-6 text-center">
        ...
        <button onclick="window.history.back()" class="mt-6 px-8 py-3 bg-[#1a0a12] border border-[#7f2f5d] text-[#ffb88c] rounded-xl text-xs font-bold uppercase tracking-widest">Return to Ledger</button>
      </div>
    `;
  }
