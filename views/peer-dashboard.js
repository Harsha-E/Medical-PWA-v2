    ...
    this.container.innerHTML = `
      <header class="view-header">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full bg-[#ca5229] flex items-center justify-center text-[var(--color-text-primary)] font-display italic font-bold shadow-lg shadow-[#ca5229]/50">${initials}</div>
          <div class="flex flex-col">
            <span class="text-xs text-[#ffb88c] uppercase font-bold tracking-[0.2em] leading-none">Remote Node</span>
            <h1 class="text-lg font-display mt-1 text-[var(--color-text-primary)] leading-none">${peerName}</h1>
          </div>
        </div>
        
        <button onclick="window.history.back()" class="w-10 h-10 rounded-full bg-white/5 border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-primary)]/70 active:scale-95 transition-transform">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
      </header>

      <main class="scroll-area px-6 pt-28 bg-[var(--color-surface-elevated)] min-h-screen">
        <section class="mb-10">
          <div class="clay-glass-panel p-6 border-[#ca5229]/30 bg-[#ca5229]/5 shadow-xl shadow-[#ca5229]/10">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="p-4 bg-black/40 rounded-2xl border border-white/5">
                <p class="text-xs text-[var(--color-text-muted)] uppercase tracking-widest mb-1">Medications</p>
                <p class="text-xl font-bold text-[var(--color-text-primary)]">${meds.length}</p>
              </div>
              <div class="p-4 bg-black/40 rounded-2xl border border-white/5">
                <p class="text-xs text-[var(--color-text-muted)] uppercase tracking-widest mb-1">Records</p>
                <p class="text-xl font-bold text-[var(--color-text-primary)]">${historyCount}</p>
              </div>
              <div class="col-span-2 p-4 bg-black/40 rounded-2xl border border-white/5 flex items-center justify-between">
                <div>
                  <p class="text-xs text-[var(--color-text-muted)] uppercase tracking-widest mb-1">Network Status</p>
          ...
          <div class="space-y-4">
            ${meds.length > 0 ? meds.map(m => `
              <div class="clay-glass-panel p-5 border-[#7f2f5d]/30 bg-white/5 flex justify-between items-center">
                <div>
                  <p class="font-bold text-[var(--color-text-primary)] text-base">${m.name}</p>
            ...
            `).join('') : `
              <div class="clay-glass-panel p-6 border-[#7f2f5d]/30 bg-white/5 text-center opacity-70">
                <p class="text-xs text-[var(--color-text-secondary)] font-mono uppercase tracking-widest">No active medications shared.</p>
              </div>
            `}
          </div>
        </section>
