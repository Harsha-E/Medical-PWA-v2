...
export default class LoginView {
  async render() {
    ...
    this.container.innerHTML = `
      ...
      <!-- Bright Overlay to lighten WebGL slightly -->
      <div class="absolute inset-0 bg-white/10 pointer-events-none z-[5] backdrop-blur-[2px]"></div>

      <div class="w-full max-w-md p-8 rounded-3xl bg-white/70 backdrop-blur-3xl border border-white/60 shadow-[0_20px_50px_rgba(0,0,0,0.15)] animate-fade-in-up relative overflow-hidden" style="box-shadow: inset 0 2px 20px rgba(255,255,255,0.8), 0 20px 50px rgba(0,0,0,0.1);">
        <!-- Inner glow -->
        <div class="absolute -top-24 -left-24 w-48 h-48 bg-white/80 blur-[50px] rounded-full pointer-events-none"></div>
        <div class="absolute -bottom-24 -right-24 w-48 h-48 bg-[#ffb88c]/30 blur-[50px] rounded-full pointer-events-none"></div>

        <div class="text-center mb-8 relative z-10">
          <div class="w-16 h-16 mx-auto bg-gradient-to-br from-white to-gray-100 rounded-2xl border border-white shadow-lg flex items-center justify-center mb-4 transition-transform hover:scale-105 duration-300">
          ...
          <h2 class="text-3xl font-display font-bold text-gray-900 tracking-tight">Access Portal</h2>
        ...
        <form id="login-form" class="space-y-5 relative z-10 group">
          <div class="relative">
            <input type="email" id="email" autocomplete="email" required class="peer w-full px-5 py-4 pt-6 rounded-xl bg-white/60 border border-white/80 text-gray-900 placeholder-transparent focus:outline-none focus:ring-2 focus:ring-[#7f2f5d]/30 focus:bg-white transition-all font-sans shadow-sm" placeholder="Clinical Identifier">
            <label for="email" class="absolute left-5 top-2 text-[10px] font-mono uppercase tracking-widest text-gray-500 transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-xs peer-placeholder-shown:normal-case peer-placeholder-shown:tracking-normal peer-focus:top-2 peer-focus:text-[10px] peer-focus:uppercase peer-focus:tracking-widest peer-focus:text-[#7f2f5d] pointer-events-none">Clinical Identifier (Email)</label>
          </div>
          
          <div class="relative">
            <input type="password" id="password" autocomplete="current-password" required class="peer w-full px-5 py-4 pt-6 rounded-xl bg-white/60 border border-white/80 text-gray-900 placeholder-transparent focus:outline-none focus:ring-2 focus:ring-[#7f2f5d]/30 focus:bg-white transition-all font-sans shadow-sm" placeholder="Security Key">
            <label for="password" class="absolute left-5 top-2 text-[10px] font-mono uppercase tracking-widest text-gray-500 transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-xs peer-placeholder-shown:normal-case peer-placeholder-shown:tracking-normal peer-focus:top-2 peer-focus:text-[10px] peer-focus:uppercase peer-focus:tracking-widest peer-focus:text-[#7f2f5d] pointer-events-none">Security Key (Password)</label>
          </div>
          
          <div class="flex justify-end">
            <button type="button" id="forgot-password" class="text-[10px] text-gray-500 font-mono uppercase tracking-widest hover:text-[#7f2f5d] transition-colors">Reset Security Key?</button>
          </div>
        ...
        <div class="relative flex items-center py-6 z-10">
          ...
          <span class="flex-shrink-0 mx-4 text-gray-400 text-[10px] font-mono uppercase tracking-widest">Federated Access</span>
          <div class="flex-grow border-t border-gray-200"></div>
        </div>

        <div class="space-y-3 relative z-10">
          <button id="google-auth" type="button" class="w-full flex items-center justify-center gap-3 py-4 rounded-xl bg-white border border-gray-200 text-gray-700 font-mono text-xs font-semibold uppercase tracking-widest hover:bg-gray-50 hover:shadow-md active:scale-95 transition-all shadow-sm">
        ...
        <p class="mt-8 text-center text-xs text-gray-500 font-mono uppercase tracking-widest relative z-10">
