    ...
    this.container.innerHTML = `
      <style>
        ...
        .google-input-group input {
          ...
          box-shadow: inset 0 2px 12px rgba(0,0,0,0.4), inset 0 -1px 2px rgba(255,255,255,0.05) !important;
      ...
      <div class="clay-glass-panel w-full max-w-md p-6 animate-fade-in-up relative z-10 border border-[var(--color-primary)]/30 shadow-[0_0_40px_rgba(255,184,140,0.15)]">
        ...
        <div id="success-container" class="hidden relative z-10 mb-5 p-3 rounded-xl border text-xs font-mono flex items-center gap-2 transition-all opacity-0 transform -translate-y-2" style="background: color-mix(in srgb, var(--color-success) 15%, transparent); border-color: var(--color-success); color: var(--color-success);">
        ...
        <form id="login-form" class="space-y-4 relative z-10 group">
          ...
          <div id="step-1" class="transition-all duration-300">
            <div class="google-input-group mb-6 mt-4">
              <input type="email" id="email" autocomplete="email" required placeholder=" " class="text-white">
              <label for="email" class="ml-2">Email</label>
              <fieldset aria-hidden="true"><legend><span>Email</span></legend></fieldset>
            </div>
            <button type="button" id="next-btn" class="w-full py-4 rounded-2xl bg-gradient-to-r from-surface-deep to-surface-elevated text-text-primary border border-primary/20 font-bold uppercase tracking-widest text-xs active:scale-95 transition-all shadow-[inset_2px_2px_4px_rgba(255,255,255,0.05),_inset_-2px_-2px_4px_rgba(0,0,0,0.5),_0_4px_20px_rgba(0,0,0,0.3)] mt-2 flex items-center justify-center gap-2">
              Next
          ...
          <div id="step-2" class="hidden transition-all duration-300 opacity-0 translate-x-4">
            <div class="flex items-center gap-3 mb-6 mt-2">
              <button type="button" id="back-btn" class="w-12 h-12 rounded-full clay-glass-panel bg-surface flex-shrink-0 flex items-center justify-center hover:-translate-x-1 transition-transform duration-300 text-text-primary shadow-[inset_2px_2px_4px_rgba(255,255,255,0.05),_inset_-2px_-2px_4px_rgba(0,0,0,0.3)]">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
              </button>
              <div class="text-xs font-mono text-text-secondary truncate flex-1 px-4 py-3 bg-surface/30 rounded-xl border border-border" id="display-email"></div>
            </div>

            <div class="google-input-group relative mb-2">
              <input type="password" id="password" autocomplete="current-password" required placeholder=" " class="text-white pr-12">
              <label for="password" class="ml-2">Password</label>
              <fieldset aria-hidden="true"><legend><span>Password</span></legend></fieldset>
              <button type="button" id="toggle-password" class="absolute right-4 top-1/2 -translate-y-1/2 text-text-secondary hover:text-text-primary focus:outline-none z-10 transition-colors p-2 rounded-full hover:bg-white/5" style="background: transparent; border: none; outline: none;">
                  <svg id="eye-icon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
              </button>
            </div>
            
            <button type="submit" id="submit-btn" class="w-full py-4 rounded-2xl bg-gradient-to-r from-surface-deep to-surface-elevated text-text-primary border border-primary/20 font-bold uppercase tracking-widest text-xs active:scale-95 transition-all shadow-[inset_2px_2px_4px_rgba(255,255,255,0.05),_inset_-2px_-2px_4px_rgba(0,0,0,0.5),_0_4px_20px_rgba(0,0,0,0.3)] mt-6 flex items-center justify-center gap-2 relative overflow-hidden">
            ...
            <button type="button" id="forgot-btn" class="w-full text-center text-[10px] font-mono uppercase tracking-widest hover:text-primary transition-colors mt-4 py-2" style="color: var(--color-text-secondary); background: transparent; border: none; box-shadow: none;">
              Forgot Password?
            </button>
          </div>
        </form>
        ...
        <div id="federated-section" class="transition-all duration-300">
          ...
          <div class="space-y-4 relative z-10">
            <button id="google-auth" type="button" class="w-full flex items-center justify-center gap-3 py-4 rounded-2xl font-mono text-xs font-semibold uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all shadow-[inset_0_2px_4px_rgba(255,255,255,0.05),_0_8px_16px_rgba(0,0,0,0.4)]" style="background: linear-gradient(145deg, rgba(30,40,60,0.6), rgba(15,20,30,0.8)); border: 1px solid rgba(255,255,255,0.1); color: var(--color-text-primary);">
  ...
  bindEvents() {
    ...
    forgotBtn.addEventListener('click', async () => {
      ...
      if (now - lastResetTime < 60000) {
        showError(`Please wait ${Math.ceil((60000 - (now - lastResetTime))/1000)}s before requesting again.`);
        return;
      }
