...
export default class GlassNavbar {
  ...
  getPublicMenu() {
    return `
      <div class="flex items-center gap-2 md:gap-3 pr-1">
        <a href="#/login" class="flex items-center gap-2 px-4 md:px-5 py-2 md:py-2.5 rounded-full text-[10px] md:text-xs font-mono tracking-widest text-gray-400 hover:text-white hover:bg-white/5 transition-all uppercase">
        ...
        <a href="#/register" class="flex items-center gap-2 px-4 md:px-6 py-2 md:py-2.5 rounded-full bg-linear-to-r from-[#7f2f5d]/80 to-[#4a1532]/80 border border-[#ffb88c]/30 text-[#ffd9b5] text-[10px] md:text-xs font-mono uppercase tracking-widest hover:brightness-125 transition-all">
  ...
  getAuthenticatedMenu(hash) {
    const navItems = [
      ...
      { href: '#/emergency', label: 'Vital', icon: '<svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"></path></svg>' },
    ...
    return navItems.map(item => {
      ...
      const colorClass = isActive ? 'text-[#ffb88c]' : 'text-gray-500 hover:text-[#ffd9b5]';
      return `
        <a href="${item.href}" class="flex-1 flex flex-row items-center justify-center px-4 py-2 transition-all ${colorClass}">
          ${item.icon}
          <span class="hidden md:block text-[10px] font-bold uppercase tracking-widest ml-2 truncate">${item.label}</span>
        </a>
      `;
    }).join('');
  }
